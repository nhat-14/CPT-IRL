...
def irl(feature_matrix, n_actions, discount, transition_probability,
    ...
    for i in pbar:
        ...
        grad_sum.append((learning_rate * grad).sum())
        pbar.set_description(desc='Gradient: {:.6f}'.format(
            (learning_rate * grad).sum()), refresh=True)

    return feature_matrix.dot(alpha).reshape((n_states,)), np.array(grad_sum)
