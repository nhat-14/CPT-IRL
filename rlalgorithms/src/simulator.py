...
from src.agents import silkmoth, infotaxis_agt, hybrid
from src.controllers import silkmoth_irl, programmed_behavior, infotaxis, itx2pb
from src.envs import smoke_video, wind_tunnel, filament
    ...
    def run(self, plume, hit_prob=1.0, draw_animation=False, save_log=False):
        ...
        with h5py.File(plume, 'r') as h5:
            if draw_animation:
                ...
                plume = h5['frames']
            #print(np.shape(plume))
            wind_data = pd.read_csv('examples/wind.csv', delimiter=',', header=None).values
